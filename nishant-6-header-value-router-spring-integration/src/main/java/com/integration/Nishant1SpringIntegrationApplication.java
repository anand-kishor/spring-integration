...
public class Nishant1SpringIntegrationApplication {
	...
	{ 
	 ...
	 @Router(inputChannel="messageOutChannel")
	 @Bean
	 public HeaderValueRouter router()
	 {
		 HeaderValueRouter router=new HeaderValueRouter("key1");
		 router.setChannelMapping("saving", "savingChannel");
	 ...
	 {
		 Map<String,HeaderValueMessageProcessor<String>> map=new HashMap<>();
		 map.put("key1",new StaticHeaderValueMessageProcessor<String>("saving"));
		 map.put("key2",new StaticHeaderValueMessageProcessor<String>("current"));
		 HeaderEnricher enricher=new HeaderEnricher(map);
		 return enricher;
	 }
	 ...
	 * 
	 * @Bean public AbstractMessageRouter msgRouter() { return new
	 * AbstractMessageRouter() {
	 * 
	 * @Override protected Collection<MessageChannel>
	 * determineTargetChannels(Message<?> message) { // TODO Auto-generated method
	 * stub Collection<MessageChannel> col=new ArrayList<>();
	 * if(message.getPayload().toString().equalsIgnoreCase("saving")) {
	 * col.add(savingChannel()); } else { col.add(defaultChannel()); } return col; }
	 * 
	 * }; }
	 */
	 @Bean
